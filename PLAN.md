...
### Sprint 2.6 — Каталог товаров + Маппинг маркетплейсов
**Даты:** май 2026

> Задача: вынести каталог товаров в отдельный раздел «Товары» с отображением статуса привязки к WB и Ozon.
> Селлер видит сразу, какие товары связаны с маркетплейсами, и может прямо в таблице привязать нужный.

#### Задачи

**[2.6-1] Backend: эндпоинт управления маппингами (`/products/:id/mappings`)**
- `POST /api/products/:id/mappings` — создать привязку (body: `{ marketplace, externalProductId, externalSku? }`)
- `DELETE /api/products/:id/mappings/:mappingId` — удалить привязку
- Используем существующую модель `ProductChannelMapping`
- Guard: `TenantWriteGuard`, проверка `tenantId` на обоих ресурсах

**[2.6-2] Backend: добавить `mappingStatus` к ответу списка продуктов**
- `findAll()` — для каждого продукта добавить `_wbMapped: boolean`, `_ozonMapped: boolean`
- Дополнительный query-param `filter=unmapped` — вернуть только товары без маппинга

**[2.6-3] Frontend: страница `/app/products` — ProductCatalog**
- Новая страница `apps/web/src/pages/ProductCatalog.tsx`
- Таблица: Фото / Название / SKU / WB статус / Ozon статус / Действия
- Колонка WB: зелёная галочка если есть `ProductChannelMapping` для WB, иначе красный крест
- Колонка Ozon: аналогично
- Бейдж «Без связки» у строки без ни одного маппинга
- Фильтр-кнопка «Только без связки»

**[2.6-4] Frontend: инлайн-форма привязки**
- При клике «Привязать» открывается строка-раскрывашка под товаром
- Форма: select маркетплейса (WB / Ozon), поле externalProductId (для WB = nmID / barcode, для Ozon = offer_id), кнопка «Сохранить»
- После сохранения — обновить строку (локально или refetch)
- Кнопка «Удалить» у существующего маппинга → DELETE запрос

**[2.6-5] Frontend: навигация**
- Добавить пункт «Товары» в `NAV_PRIMARY` в `MainLayout.tsx` (маршрут `/app/products`)
- «Учёт остатков» (`/app/inventory`) остаётся, но убран из основного меню → в «Операции»
- Обновить `App.tsx` — добавить маршрут `/app/products`

#### ✅ Результат спринта
- Раздел «Товары» показывает статус WB / Ozon для каждого товара
- Товары без связки визуально выделены и легко фильтруются
- Продавец может привязать товар к маркетплейсу прямо в таблице

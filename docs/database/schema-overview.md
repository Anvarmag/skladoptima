...
- `tenant_id` — принадлежность к тенанту. Все запросы к товарам обязательно фильтруются по этому полю.
- `name` — название товара, отображаемое в каталоге.
- `description` — описание товара. Необязательное.
- `brand` — бренд/производитель. Используется для фильтрации и аналитики.
- `category` — категория товара (свободный текст). Позволяет группировать товары в учёте.
- `status` — `active` (в продаже), `archived` (товар неактуален, скрыт из магазина), `draft` (черновик, ещё не опубликован).
- `main_photo_url` — URL главного фото в S3. Сам файл хранится в объектном хранилище, в БД — только ссылка.
- `created_by_user_id` — кто из пользователей добавил товар (учитывается в аудите).
- `created_at` / `updated_at` / `deleted_at` — служебные метки; `deleted_at` для soft delete.

**Индексы:**
- `INDEX (tenant_id, status)` — фильтрация активных товаров
- `INDEX (tenant_id, created_at)` — сортировка по дате добавления
...
**Описание полей:**
- `id` — уникальный идентификатор SKU.
- `product_id` — к какому товару относится эта SKU.
- `tenant_id` — денормализация: тенант хранится здесь для быстрой фильтрации без JOIN через products.
- `sku_code` — внутренний артикул пользователя (например, SHIRT-RED-XL). Уникален в рамках тенанта.
- `barcode` — штрихкод / EAN-13. Используется для сопоставления с карточками WB/Ozon и сканирования на складе.
- `name` — название вариации (например, «Футболка красная, размер XL»).
- `attributes_json` — атрибуты в формате JSON: `{"color": "red", "size": "XL"}`. Гибкое хранилище для любых характеристик.
- `photo_url` — URL фото вариации в S3 (может отличаться от главного фото товара).
- `status` — `active` (активна) или `archived` (архивирована, не участвует в синке).
- `created_at` / `updated_at` / `deleted_at` — служебные метки.

**Индексы:**
- `INDEX (tenant_id, barcode)` — поиск по баркоду в рамках тенанта
- `INDEX (product_id)` — все SKU товара
...
**Описание полей:**
- `id` — уникальный идентификатор внутреннего склада.
- `tenant_id` — владелец склада. Все остатки на этом складе принадлежат этому тенанту.
- `name` — название склада (например, «Главный склад», «Офис Москва»). Создаётся пользователем.
- `address` — физический адрес склада (опционально). Полезен для документов, логистики и внутренних записей.
- `is_default` — основной склад по умолчанию для этого тенанта. Используется в UI и при автоматической привязке остатков.
- `marketplace_warehouse_id` — связь с конкретным складом маркетплейса (если это FBS-модель и склад привязан к WB/Ozon-складу).
- `status` — `active` или `archived`. Архивные склады не участвуют в синхронизации и не показываются в UI.
- `created_at` / `updated_at` — служебные метки времени.
...
**Описание полей:**
- `id` — уникальный идентификатор строки остатка.
- `tenant_id` — принадлежность к тенанту.
- `sku_id` — конкретная SKU, остаток которой хранится.
- `warehouse_id` — на каком складе находится товар.
- `quantity` — текущее физическое количество на складе. Обновляется при каждом движении.
- `reserved_quantity` — зарезервированное количество (например, заявлено в ожидающих заказах). Нельзя продать или отгрузить без отмены резерва.
- `available_quantity` — **вычисляемое поле** (`quantity - reserved_quantity`). Актуальный доступный остаток. Именно это значение отправляется на WB/Ozon.
- `last_sync_at` — когда остаток был синхронизирован с маркетплейсом. Чем дольше от NOW() — тем старше данные на площадке.
- `created_at` / `updated_at` — служебные метки.

**Индексы:**
- `UNIQUE (tenant_id, sku_id, warehouse_id)` — одна строка = один SKU на одном складе
- `INDEX (tenant_id, warehouse_id)` — все SKU на складе
...
- `tenant_id` / `sku_id` / `warehouse_id` — что, чего и где движется. Все три поля обязательны.
- `type` — тип движения: `inbound` (поступление товара), `outbound` (отгрузка), `adjustment` (ручная корректировка, например после инвентаризации), `sync_correction` (исправление расхождений после синка), `reservation` (резерв под заказ), `reservation_cancel` (отмена резерва), `transfer` (перемещение между складами).
- `quantity_before` — количество до изменения. Нужно для аудита и восстановления истории.
- `quantity_change` — изменение (может быть отрицательным). Например: `-5` (отгрузили), `+10` (пришло).
- `quantity_after` — количество после изменения. Должно всегда совпадать с `stock_balances.quantity`.
- `adjustment_reason_id` — причина коррекции (обязательна при `type = adjustment`).
- `performed_by_user_id` — кто выполнил действие (пользователь или система). `NULL` — автоматическое действие.
- `source` — инициатор изменения: `manual` (вручной ввод), `marketplace_sync` (автосинк), `import` (импорт из файла), `api` (через API).
- `reference_id` — ID связанного документа (например, ID заказа, импорта или запуска синка). Для связи с источником движения.
- `notes` — произвольный комментарий пользователя.
- `created_at` — дата создания. **Никогда не обновляется** — append-only.

**Индексы:**
- `INDEX (tenant_id, sku_id, created_at)` — история движений по SKU
- `INDEX (tenant_id, warehouse_id, created_at)` — все движения на складе
...
- `tenant_id` — если `NULL` — это системная причина, доступная всем тенантам. Если заполнено — пользовательская причина, видна только этому тенанту.
- `name` — название причины (например, «Пересчёт», «Брак», «Утеря»). Отображается пользователю в форме коррекции.
- `is_system` — `true` — системная причина, нельзя редактировать или удалить. `false` — пользовательская.
- `created_at` — дата создания записи.
...
**Описание полей:**
- `id` — уникальный идентификатор финансового профиля.
- `tenant_id` — принадлежность к тенанту.
- `sku_id` — SKU, для которой заполнены затраты.
- `marketplace_account_id` — к какому кабинету привязан профиль (у WB и Ozon разные комиссии).
- `purchase_price` — закупочная цена единицы товара. Основа для расчёта себестоимости.
- `logistics_cost` — стоимость доставки товара до склада WB/Ozon. Включается в себестоимость.
- `marketplace_commission` — комиссия маркетплейса в процентах (например, `15` = 15%). Зависит от категории.
- `storage_cost` — стоимость хранения на складе маркетплейса в рублях за единицу.
- `packaging_cost` — затраты на упаковку (коробка, плёнка и т.д.).
- `return_rate` — ожидаемый процент возвратов (например, `5` = 5%). Влияет на расчёт чистой прибыли.
- `rating_bonus_cost` — расходы на поддержание рейтинга: выкупы, отзывы.
- `tax_rate` — налоговая ставка в процентах (например, `6` для УСН 6%).
- `selling_price` — цена продажи товара покупателю.
- `dimensions_json` — габариты и вес: `{"weight": 0.5, "length": 10, "width": 8, "height": 5}`. Нужны для расчёта логистики.
- `currency` — валюта всех числовых полей, по умолчанию `RUB`.
- `valid_from` / `valid_until` — период действия профиля. Можно вести историю изменения цен/затрат.
- `created_at` / `updated_at` — служебные метки времени.
...
- `tenant_id` — принадлежность к тенанту.
- `sku_id` — SKU, для которой рассчитана экономика.
- `finance_profile_id` — финансовый профиль, на основе которого сделан расчёт.
- `calculated_at` — момент расчёта. Снимки создаются периодически (например, раз в сутки worker-ом).
- `revenue` — выручка (selling_price × количество проданных единиц).
- `total_costs` — сумма всех затрат: закупка + логистика + хранение + упаковка + возвраты.
- `gross_profit` — валовая прибыль: `revenue - total_costs`.
- `gross_margin` — маржинальность в процентах: `gross_profit / revenue × 100`.
- `net_profit` — чистая прибыль после вычета налогов.
- `net_margin` — чистая маржинальность в процентах.
- `roi` — возврат на инвестиции: `net_profit / purchase_price × 100`.
- `meta_json` — детали расчёта для отладки: детализация каждой статьи затрат.
...
- `tenant_id` — к какому тенанту относится событие. `NULL` для системных событий.
- `actor_user_id` — кто выполнил действие. `NULL` — автоматический процесс (worker, синк).
- `action` — код действия в формате `устройство.действие`: `stock.adjusted`, `member.invited`, `marketplace.connected`. Используется для фильтрации в аудит-логе.
- `entity_type` — тип изменённой сущности: `sku`, `membership`, `marketplace_account`, `tariff` и т.д.
- `entity_id` — ID конкретной изменённой записи. Вместе с `entity_type` позволяет найти всю историю изменений конкретного объекта.
- `before_json` — состояние сущности до действия. Нужно для понимания, что именно изменилось.
- `after_json` — состояние после. Сравнение `before` и `after` показывает дифф.
- `ip_address` — IP пользователя в момент действия. Используется для безопасности.
- `user_agent` — браузер/устройство. Помогает заметить смену устройства и необычные паттерны.
- `source` — откуда пришло действие: `web` (через интерфейс), `api` (внешний API), `worker` (автоматическая задача), `admin` (админ через панель), `system` (система).
- `created_at` — дата события. **Никогда не меняется** — append-only.

**Индексы:**
- `INDEX (tenant_id, action, created_at)` — выборка событий по типу
- `INDEX (tenant_id, entity_type, entity_id)` — история изменений объекта
- `INDEX (actor_user_id, created_at)` — все действия пользователя
...
**Описание полей:**
- `id` — уникальный идентификатор события безопасности.
- `user_id` — пользователь, с которым связано событие. `NULL` для анонимных попыток.
- `event_type` — тип события: `login_success` (успешный вход), `login_failed` (неудачная попытка), `password_reset_requested` (запрос сброса), `password_changed` (пароль изменён), `email_change_requested` (запрос смены email), `mfa_enabled` (включена 2FA), `suspicious_login` (подозрительный вход, например из новой страны), `account_blocked` (аккаунт заблокирован).
- `ip_address` — IP в момент события. Нужен для обнаружения брутфорса и необычных геолокаций.
- `user_agent` — браузер и устройство. Помогает заметить смену устройства и необычные паттерны.
- `geo_country` — страна по IP. Позволяет выявить вход из новой страны и отправить уведомление пользователю.
- `risk_score` — оценка риска от 0 до 100. Рассчитывается автоматически (новый IP, новая страна, много попыток). При высоком скоре — алерт.
- `resolved` — обработано ли расследование события. Позволяет команде безопасности отмечать обработанные события.
- `notes` — внутренние пометки команды по событию.
- `created_at` — время события. Append-only.
...
- `tenant_id` — от какого тенанта поступило обращение. `NULL` для пользователей, не вошедших в тенант.
- `user_id` — пользователь, отправивший обращение.
- `type` — тип обращения: `general` (общий), `store_reassignment` (перепривязка кабинета), `billing` (вопрос по оплате), `technical` (техническая проблема), `fraud` (подозрение в мошенничестве).
- `status` — жизненный цикл: `open` (открыто), `in_progress` (в работе), `resolved` (решено), `closed` (закрыто).
- `subject` — тема обращения, отображается в очереди саппорта.
- `description` — подробное описание проблемы от пользователя.
- `assigned_to_user_id` — на кого из команды назначено обращение (support-агент).
- `resolved_at` — когда обращение было закрыто.
- `created_at` / `updated_at` — служебные метки.
...
**Описание полей:**
- `id` — уникальный идентификатор заявки на перепривязку.
- `marketplace_account_external_id` — ID кабинета в WB/Ozon, который хотят перепривязать.
- `marketplace` — площадка: `wildberries` или `ozon`.
- `requesting_tenant_id` — тенант, который хочет подключить себе этот кабинет.
- `current_tenant_id` — тенант, у которого кабинет привязан сейчас. `NULL` если невозможно определить.
- `status` — `pending` (ждёт обработки), `approved` (перепривязка одобрена support), `rejected` (отказано), `auto_resolved` (решено автоматически, например если текущий тенант отключил кабинет).
- `verification_method` — как подтвердили владение кабинетом: `api_key_check` (проверка ключа), `document_upload` (загрузка документов) и т.д.
- `handled_by_user_id` — support-агент, разрешивший заявку. `NULL` если автоматическое разрешение.
- `handled_at` — когда заявка была обработана.
- `notes` — внутренние заметки support-агента (причина решения, дополнительный контекст).
- `created_at` / `updated_at` — служебные метки.
